...
class maxbotsFrontController extends waFrontController
{
        ...
        if ($env == 'backend') {
            ...
            if (empty($module) && empty($plugin)) {
                $routing = new waRouting($this->system, array(
                    'default' => array(
                        array(
                            'url' => wa()->getConfig()->systemOption('backend_url').'/maxbots/*',
                            'app' => 'maxbots',
                        ),
                    ),
                ));
    ...
    protected function runController($controller, $params = null)
    {
        ...
        $handled = wa('maxbots')->event('controller_before.'.$class, $evt_params);
        if ($handled) {
            return;
        }
        $result = parent::runController($controller, $params);
        wa('maxbots')->event('controller_after.'.$class, $params);
        return $result;
    }
}
